...
is an independent voice. A snake of lit LEDs cascades downward through the column and
wraps around in a continuous loop; the gate output for that column fires at the moment the
snake's head appears at the top, and stays open until the tail exits the top border.

The eight rows select the speed of the snake. Row A (top) is the fastest; row H (bottom)
...
- **Start a column:** press and release a speed row button (rows A–G) while the column is
  off. The gate fires immediately on the first loop. The snake length defaults to 1.
...
- **Set snake length (1–7 rows):** hold one row and press a second row in the same column.
...
**snake color** and an optional **background color** that dims active columns.
